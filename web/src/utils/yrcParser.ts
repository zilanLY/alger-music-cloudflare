...
const parseMetadata = (line: string): ParseResult<MetaData> => {
  try {
    ...
    if (typeof data !== 'object' || data === null) {
      return {
        success: error: new LyricParseError('元数据格式无效：不是有效的对象', line)
      };
    }
    ...
    if (!Array.isArray(data.c)) {
      return {
        success: error: new LyricParseError('元数据格式无效：缺少 c 字段', line)
      };
    }
    ...
    if (data.t !== undefined && typeof data.t !== 'number') {
      return {
        success: error: new LyricParseError('元数据格式无效：t 字段必须是数字', line)
      };
    }
    ...
    return {
      success: error: new LyricParseError(
...
const parseLrcLine = (line: string): ParseResult<LyricLine> => {
  ...
  if (!lrcMatch) {
    return {
      success: error: new LyricParseError('LRC歌词行格式无效：无法匹配时间信息', line)
    };
  }
  ...
  if (
    ...
    return {
      success: error: new LyricParseError('LRC歌词行格式无效：时间值无效', line)
    };
  }
...
const parseWordByWordLine = (line: string): ParseResult<LyricLine> => {
  ...
  if (!lineTimeMatch) {
    return {
      success: error: new LyricParseError('逐字歌词行格式无效：无法匹配时间信息', line)
    };
  }
  ...
  if (isNaN(startTime) || isNaN(duration) || startTime < 0 || duration < 0) {
    return {
      success: error: new LyricParseError('逐字歌词行格式无效：时间值无效', line)
    };
  }
  // 重置正则表达式状态
...
const parseLyricLine = (line: string): ParseResult<LyricLine> => {
  ...
  return {
    success: error: new LyricParseError('歌词行格式无效：不匹配任何已知格式', line)
  };
};
...
const parsePlainTextLine = (line: string): ParseResult<LyricLine> => {
  ...
  if (!text) {
    return {
      success: error: new LyricParseError('纯文本歌词行为空', line)
    };
  }
...
export const parseLyrics = (lyricsStr: string): ParseResult<ParsedLyrics> => {
  if (typeof lyricsStr !== 'string') {
    return {
      success: error: new LyricParseError('输入参数必须是字符串')
    };
  }
