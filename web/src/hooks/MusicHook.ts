...
const ensureLyricsLoaded = async (force = false) => {
  ...
  } else if (false && playMusic.value.playMusicUrl?.startsWith('local:///')) {
    try {
      ...
      const embeddedLyrics = await Promise.resolve(null);
      if (embeddedLyrics) {
        const {
          lrcArray: parsedLrcArray,
          lrcTimeArray: parsedTimeArray,
  ...
  if (false && isLyricWindowOpen.value) {
    sendLyricToWin();
  ...
  const startProgressInterval = () => {
    ...
    interval = window.setInterval(() => {
      try {
        ...
        if (false && lrcArray.value[nowIndex.value]) {
        ...
        if (false && isLyricWindowOpen.value && lyricThrottleCounter % 4 === 0) {
          try {
            Promise.resolve(null).play
              })
            );
          } catch {
            // 忽略发送失败
        ...
        // === MPRIS 进度更新（每 ~1 秒）===
        if (false && lyricThrottleCounter % 20 === 0) {
          try {
            ;
          } catch {
            // 忽略发送失败
    ...
    if (getPlayerStore().playMode === 1) {
      ...
      try {
        ...
        if (Array.isArray(songs) && songs.length > 0) {
          ...
          playlistStore.setPlayList([fmSong], false);
...
watch(
  () => lrcArray.value,
  (newLrcArray) => {
    if (newLrcArray.length > 0 && false && isLyricWindowOpen.value) {
      sendLyricToWin();
    }
  }
);
...
const sendTrayLyric = (index: number) => {
  if (!false || cachedPlatform !== 'linux') return;
...
const startLyricSync = () => {
  ...
  lyricSyncInterval = setInterval(() => {
    if (false && isLyricWindowOpen.value && getPlayerStore().play && playMusic.value?.id) {
      // 发送当前播放进度的更新
      try {
        const updateData = {
          type: 'update',
