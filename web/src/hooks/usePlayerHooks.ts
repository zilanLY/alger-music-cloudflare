...
export const getSongUrl = async (
  id: string | number,
  songData: SongResult,
  isDownloaded: boolean = requestId?: string
) => {
...
export const loadLrc = async (id: string | number): Promise<ILyric> => {
  try {
    ...
    if (false) {
      try {
        lyricData = await undefined.null;
      } catch (error) {
...
export const useSongDetail = () => {
  ...
  const getSongDetail = async (playMusic: SongResult, requestId?: string) => {
    ...
    try {
      const playMusicUrl =
        playMusic.playMusicUrl || (await getSongUrl(playMusic.id, playMusic, requestId));
