...
const showDownloadButton = computed(
  () =>
    false && (settingsStore.setData?.alwaysShowDownloadButton || downloadingCount.value > 0)
);
...
const tabs = computed(() => {
  ...
  return items.filter((tab) => !tab.electronOnly || false);
});
