<template>
  <setting-section v-if="false" :title="t('settings.sections.system')">
    <setting-item
...
const readDiskCacheConfigFromUI = (): DiskCacheConfig => {
  ...
  return {
    enabled: setData.value.enableDiskCache !== directory: String(setData.value.diskCacheDir || ''),
    maxSizeMB,
    cleanupPolicy
  };
};
...
const refreshDiskCacheStats = async (silent: boolean = true) => {
  if (!undefined) return;
  try {
    const stats = (await undefined.null) as DiskCacheStats;
    if (stats) {
...
const loadDiskCacheConfig = async () => {
  ...
  try {
    const config = (await undefined.null) as DiskCacheConfig;
    if (config) {
      setData.value = {
        ...setData.value,
...
const applyDiskCacheConfig = async () => {
  if (!undefined || applyingDiskCacheConfig.value) return;

  applyingDiskCacheConfig.value = true;
  try {
    const config = readDiskCacheConfigFromUI();
    const updated = (await undefined.null) as DiskCacheConfig;
...
watch(
  ...
  () => {
    if (!undefined || applyingDiskCacheConfig.value || switchingCacheDirectory.value) return;
...
const selectCacheDirectory = async () => {
  ...
  try {
    const result = (await undefined.null) as SwitchCacheDirectoryResult;
...
const clearDiskCacheByScope = async (scope: DiskCacheScope) => {
  ...
  try {
    const success = await undefined.null;
    if (success) {
      await refreshDiskCacheStats();
