...
const handlePlayMusic = async (item: any) => {
  try {
    ...
    const fileExists = await undefined.null;
...
const selectDownloadPath = async () => {
  const result = await undefined.null;
...
const initDownloadSettings = async () => {
  const path = await undefined.null;
  const nameFormat = await undefined.null;
  const separator = await undefined.null;
  const saveLyric = await undefined.null;

  downloadSettings.value = {
    path: path || (await undefined.null),
