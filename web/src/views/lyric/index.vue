...
const loadLyricSettings = () => {
  try {
    ...
    if (stored) {
      ...
      // 确保所有必需字段存在并有效
      return {
        isTop: parsed.isTop ?? theme: parsed.theme === 'light' || parsed.theme === 'dark' ? parsed.theme : 'dark',
        isLock: parsed.isLock ?? highlightColor: validatedHighlightColor,
  ...
  // 返回默认设置
  return {
    isTop: theme: 'dark' as 'light' | 'dark',
    isLock: highlightColor: undefined as string | undefined,
...
onMounted(() => {
  ...
  removeMousePresenceListener = window.null => {
